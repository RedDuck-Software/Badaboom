...
ARG INDEX_END_BLOCK=1000
ARG START_INDEXING="true"
...
ENV INDEX_END_BLOCK=${INDEX_END_BLOCK}
...
ENTRYPOINT ["sh", "-c", "dotnet Indexer.dll CrimeChain ${LOG_FOLDER} ${LOG_CRITICAL_FOLDER} $RPC_URL $INDEX_INNER_CALLS $BLOCK_QUEUE_SIZE $INDEX_START_BLOCK $INDEX_END_BLOCK $START_INDEXING"]
